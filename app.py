...
def _extract_relevant_lines(content: str, query: str, max_lines: int = 6) -> str:
    lines = [line.strip() for line in str(content).splitlines() if line.strip()]
    if not lines:
        return str(content).strip()

    q_tokens = [tok for tok in query.replace('/', ' ').split() if tok]
    ...
    for idx, line in enumerate(lines):
        ...
        for tok in q_tokens:
            if tok in line:
                score += 2
            if _compact_text(tok) in line_compact:
                score += 1
        if score > 0:
            matched.append((score, idx, line))
...
def find_answer(user_text: str) -> str:
    ...
    for menu_name, data in manuals.items():
        ...
        score = 0
        menu_compact = _compact_text(menu_name)
        title_compact = _compact_text(title)
        content_compact = _compact_text(content)

        if q == menu_name or q == title:
            score += 100
        if q_compact and q_compact in menu_compact:
            score += 60
        if q_compact and q_compact in title_compact:
            score += 50
        if q_compact and q_compact in content_compact:
            score += 35

        for token in [tok for tok in q.replace('/', ' ').split() if tok]:
            token_compact = _compact_text(token)
            if token in menu_name or token_compact in menu_compact:
                score += 8
            if token in title or token_compact in title_compact:
                score += 7
            if token in content or token_compact in content_compact:
                score += 5

        matched_faq = None
        for item in faq:
            if isinstance(item, (list, tuple)) and len(item) >= 2:
                fq, fa = str(item[0]), str(item[1])
                fq_compact = _compact_text(fq)
                fa_compact = _compact_text(fa)
                faq_score = 0
                if q_compact and q_compact in fq_compact:
                    faq_score += 45
                if q_compact and q_compact in fa_compact:
                    faq_score += 20
                for token in [tok for tok in q.replace('/', ' ').split() if tok]:
                    token_compact = _compact_text(token)
                    if token in fq or token_compact in fq_compact:
                        faq_score += 8
                    if token in fa or token_compact in fa_compact:
                        faq_score += 3
                if faq_score > score:
                    score = faq_score
                    matched_faq = (fq, fa)
        ...
        if score > best_score:
            best_score = score
            best_match = (menu_name, content, matched_faq)

    if best_match and best_score >= 8:
        menu_name, content, matched_faq = best_match
        if matched_faq:
            fq, fa = matched_faq
            return f"[{menu_name}]\nQ. {fq}\nA. {fa}"

        snippet = _extract_relevant_lines(content, q)
        return f"[{menu_name}]\n{snippet}"
